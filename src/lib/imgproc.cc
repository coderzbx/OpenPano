...
void resize_bilinear(const Matuc &src, Matuc &dst) {
  vector<int> tabsx(dst.rows());
  vector<int> tabsy(dst.cols());
  vector<unsigned char> tabrx(dst.rows());
  vector<unsigned char> tabry(dst.cols());
  ...
  const int ch = src.channels();
  for (int dx = 0; dx < dst.rows(); ++dx) {
    const unsigned char *p0 = src.ptr(tabsx[dx]+0);
    const unsigned char *p1 = src.ptr(tabsx[dx]+1);
    unsigned char *pdst = dst.ptr(dx);
    float rx = tabrx[dx], irx = 1.0f - rx;
    for (int dy = 0; dy < dst.cols(); ++dy) {
      ...
      unsigned char ry = tabry[dy], iry = 1.0f - ry;
      for (int c = 0; c < ch; ++c) {
        unsigned char res = rx * (pc11[c]*ry + pc10[c]*iry)
            + irx * (pc01[c]*ry + pc00[c]*iry);
        pcdst[c] = res;
      }
    }
...
template <>
void resize<unsigned char>(const Matuc &src, Matuc &dst) {
  m_assert(src.rows() > 1 && src.cols() > 1);
  m_assert(dst.rows() > 1 && dst.cols() > 1);
  m_assert(src.channels() == dst.channels());
  m_assert(src.channels() == 1 || src.channels() == 3);
